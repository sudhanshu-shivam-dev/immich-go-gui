...
def build_archive_icloud_tab(host) -> QWidget:
    page = QWidget()
    lay = QVBoxLayout(page)
    lay.setContentsMargins(0, 16, 0, 0)
    lay.setSpacing(24)
    host.inputs["archive-icloud"] = {}
    ...
    host.inputs["archive-icloud"]["path"] = p_edit

    btn_folder = QPushButton("Select Folder…")
    btn_folder.setCursor(Qt.CursorShape.PointingHandCursor)
    btn_folder.clicked.connect(host.browse_folder_archive_icloud)

    btn_zip = QPushButton("Select ZIP Archive…")
    btn_zip.setCursor(Qt.CursorShape.PointingHandCursor)
    btn_zip.clicked.connect(host.browse_zip_archive_icloud)
    ...
    arch_icloud_err = host._make_field_error_label()
    p_layout.addWidget(arch_icloud_err)
    host._register_field_error_label("archive-icloud", "path", arch_icloud_err)
    host._bind_field_error_clear("archive-icloud", "path", p_edit)
    ...
    host.inputs["archive-icloud"]["write-to"] = t_write
    host._add_browse_action(t_write, "Select Archive Destination")
    form.add_row(
        "Destination Folder",
        host._wrap_with_field_error("archive-icloud", "write-to", t_write),
    )

    card.layout.addLayout(form)
    lay.addWidget(card)

    adv_card = host._build_advanced_flags_card("archive-icloud")
